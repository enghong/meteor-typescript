  ...
  _createClass(TSBuild, [{
    ...
    value: function emit(filePath) {
      ...
      var result = compileCache.get(filePath, csOptions, function (cacheResult) {
        ...
        var csResult = (0, _compileService.createCSResult)(filePath, cacheResult);
        var tsDiag = csResult.diagnostics;
        var unresolved = tsDiag.hasUnresolvedModules();
        if (unresolved || refsChange !== _refs.RefsChangeType.NONE || isTypingsChanged) {
          _logger2.default.debug("diagnostics re-evaluation: %s", filePath);
          var pdiag = _logger2.default.newProfiler("diags update");
